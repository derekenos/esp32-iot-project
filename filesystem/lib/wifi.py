...
def connect_to_access_point(ssid, password):
    ...
    if not wlan.active() or not wlan.isconnected():
        wlan.active(True)
        wlan.config(dhcp_hostname=config['DHCP_HOSTNAME'].format(
            UNIQUE_ID=UNIQUE_ID))
        print('connecting to:', ssid)
        wlan.connect(ssid, password)
        wait_seconds_remaining = config['WLAN_CONNECT_WAIT_SECONDS']
...
def create_access_point():
    ...
    ap.config(essid=config['WIFI_ACCESS_POINT_SSID'].format(
        UNIQUE_ID=UNIQUE_ID))
    ap.active(True)
    return ap
